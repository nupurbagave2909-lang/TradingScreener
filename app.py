    ...
    for name, token in indices.items():
        try:
            ...
            if d['status']:
                ...
                if name == "INDIA VIX": vix_value = ltp
...
# --- UI ---
st.sidebar.title("🛡️ Secure Login")
...
u_sec = st.sidebar.selectbox("Sector", ["Auto-Detect"] + list(STOCKS_DB.keys()))
                    ...
                    if i + j < len(stocks):
                        ...
                        if h_res['status'] and len(h_res['data']) >= 2:
                            ...
                            if c_res['status'] and c_res['data']:
                                ...
                                with cols[j]:
                                    if signal:
                                        if lottie_success: st_lottie(lottie_success, height=80, key=f"rock_{s['s']}")
                                        st.success(f"🎯 **SIGNAL: {s['s']}**")
                                        e1, e2, e3 = st.columns(3)
                                        e1.metric("ENTRY", f"₹{signal['ent']}")
